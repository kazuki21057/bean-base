...
## L113 `.claude/agents/`のサブエージェント定義はCLIプロセス起動時にしか読み込まれない(`/clear`では反映されない)

2026-08-05のT3-81(サブエージェント委譲ルールの整備)で、`.claude/agents/architect.md`を新規作成した直後に`subagent_type: architect`で疎通確認を試みたところ、`Agent type 'architect' not found. Available agents: claude, claude-code-guide, Explore, general-purpose, Plan, statusline-setup`というエラーになった。同じセッションの**前**(同日17:08、別セッション)に作成済みだった`implementer`・`verifier`も同様に一覧に出ていない。当セッションは`/clear`で会話をクリアして開始したものであり、**`/clear`は会話履歴を消すだけでCLIプロセスを再起動しないため、エージェントレジストリは古いまま**だった。

**教訓**: `.claude/agents/*.md`を新規作成・改名したら、**そのセッションでは`subagent_type`に指定できない**(定義ファイルのfrontmatterが正しくても`not found`になるので、書式ミスと切り分けがつかない)。反映にはClaude Codeの**プロセス再起動が必要**で、`/clear`では不十分。作成した当セッションでどうしても委譲したい場合は`general-purpose`+`model:`パラメータで代用する(この代用ではエージェント定義の本文=役割指示が渡らないので、必要な規約はプロンプトに書き出すこと)。逆に、委譲ルールを整備するタスクでは「ルールを書いた=次から動く」ではなく、**次セッション冒頭でシステムリマインダーの`Available agent types`一覧に名前が載っているかを目視確認する**まで完了としない。定義の内容(frontmatterの妥当性)を検証したいときも、一覧に載ってから最小プロンプトで疎通させる。
