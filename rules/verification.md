...
- L113 `.claude/agents/*.md`はCLIプロセス起動時にしか読み込まれず、**`/clear`では反映されない**。新規作成した当セッションでは`subagent_type`指定が`not found`になる(書式ミスと切り分け不能)。次セッション冒頭の`Available agent types`一覧に載っているか目視確認するまで完了としない
