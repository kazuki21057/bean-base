...
2. `loop_guard.js`(UserPromptSubmitフック)がユーザープロンプト送信のたびに`[loop_guard] 本ループ cost=.../turns=.../fails=...`という形でフック出力を表示するので、その値をそのまま真値として使い、本ループのコスト・ターン数・連続失敗回数を確認する(`.claude/loop_state.md`・`.claude/loop_failures.txt`をReadし直す必要はない。2026-07-25〜、しきい値の集計単位は当日累計ではなく直近の`/start`・`/full_loop`以降の1ループ単位)。しきい値(コスト$24超・ターン30到達・連続失敗3回)にすでに達している場合は、新規タスクに着手せず、その旨をユーザーに伝えて指示を仰ぐ。
