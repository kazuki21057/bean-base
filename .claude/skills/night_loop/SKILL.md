...
2. **タスク選定**
   ...
   - 選んだタスクが**S/Mサイズ**の場合: そのタスクを手順3〜5(実装→検証→自動pushゲート判定)まで完了させたあと、**新規着手前に`.claude/loop_state.md`のコスト・ターン数・連続失敗回数を再確認**し、夜間のしきい値(コスト$20/ターン80/連続失敗2)に余裕があれば、依存が満たされた次点のS/Mタスクをもう1件選び、同様に手順3〜5を実施する(**合計最大2件まで**。3件目には進まない)。次点の候補がLサイズまたは「⚠️上位モデルで実施」しか残っていない場合は2件目は選ばず、1件目の結果のみで本ループを終える。
   - 該当するS/M/Lタスクが1件も無い(全件が依存未充足、または残りが「⚠️上位モデルで実施」のみ)場合は、新規タスクを発明せず**§中断条件**へ。
