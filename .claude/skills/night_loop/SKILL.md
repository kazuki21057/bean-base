...
2. **タスク選定**
   ...
   - 依存が満たされた最上位のタスクを1件選ぶ。**S/M/Lいずれのサイズも対象**とする(「⚠️上位モデルで実施」タスクは引き続き対象外、設計判断を伴うため無人実行に向かないという理由は変更なし)。
   ...
   - 該当するS/M/Lタスクが1件も無い(全件が依存未充足、残りが「⚠️上位モデルで実施」のみ、またはオープンPR確認で全て除外された)場合は、新規タスクを発明せず**§中断条件**へ。
...
1. 着手できるS/M/Lタスクが無い(全件が依存未充足、または残りが「⚠️上位モデルで実施」のみ)
