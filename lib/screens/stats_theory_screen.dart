  ...
  Widget _gpSection() {
    return _sectionCard(
      ...
      children: const [
        ...
        _SubHead('「探索の検証状況」画面(045)での読み方'),
        _Para('EI(期待改善量)の最大値は、今の最高スコアをどれだけ上回れそうかの期待値で、'
            'まだ試していない・不確かな条件ほど大きくなります。目安として3段階に分けており、'
            '0.20以上なら「まだ試す価値のある条件が残っている」、0.05以上0.20未満なら'
            '「かなり探索できている(あと少し)」、0.05未満なら「ほぼ探索し尽くした」と読めます。'
            '総合評価は1〜10の整数入力なので、0.05点は入力粒度(1点)の1/20に相当し、'
            'これを下回るとグリッド上にもう伸びしろが見えない状態とみなします。'),
        _Para('045のヒートマップで塗られる色は、産地や焙煎度が近い他の豆の記録も学習に使った'
            'GPの予測値です。一方●は、その豆で実際に試した回数(実測)を表します。'
            '●の付いていないマスにも色(予測値)が出ますが、これは近い条件の記録から'
            '補間しているだけで、異常ではありません。'),
      ],
    );
  }
