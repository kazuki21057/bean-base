...
## 8. Proプラン使用率ログ(2026-08-09追加、2026-08-09にローカルAPI取得へ移行)

`/full_loop`の開始時・終了時に`curl http://localhost:3000/`(ユーザーが立てたローカル使用状況確認APIサーバー、レスポンス例「Current session: 100% used · resets Aug 9, 9:49pm」「Current week (all models): 66% used · resets Aug 13, 4:59am」)を実行して自動取得し、ここに蓄積する。**現時点ではUbuntu環境のみ起動**(Windows側で起動され次第、両OSで取得する運用に更新する)。サーバ未起動の場合はユーザーの口頭申告で代替する(従来方式)。目的は`loop_guard`のコスト実測($)とプラン使用率(%)の対応関係を積み上げ、%からコスト・コストから%への換算精度を上げること。**取得も申告も無いループは記録しない**(推測値を書かない)。
