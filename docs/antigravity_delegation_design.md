...
| `researcher` | **パイロット再開(2026-08-14、T5-A79により条件を差し替え)**。`tools/verify_citations.ps1`による出典の機械検証(URLの実在+引用の原文照合)を**必ず通すこと**を条件に、researcher専用除外条件(下記)に当たらない一般調査をagyへ出してよい。ラッパーが自動で検証し、1行でも失敗すれば exit 18(`CITATION_UNVERIFIED`)でフォールバック扱いになる。**親が手作業でWebFetchして出典を確認する運用(T5-A75の暫定条件)は、この機械検証に置き換える**。昇格判定(§9.5)は従来どおり3件中2件以上で行い、T5-A79の2件(pro/flash)を1件目・2件目として数える |
...
**現時点(T5-A83未実装)では従来表がそのまま有効**。下表は各パイロットの完了をもって段階的に置き換える。
