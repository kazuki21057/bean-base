...
- UI: `lib/widgets/statistics/` に新規ウィジェット追加、`lib/screens/statistics_screen.dart` の既存セクション列 (Filter → KPI → Radar → PCA → Ranking) の後ろに挿入。F3 のみダッシュボード (`AppScreen.dashboard`) に配置。
...
#### 3.4.2 GAS ソースのリポジトリ内管理 (Q-E 対応)

今後 Claude Code が GAS を編集できるよう、GAS ソースをリポジトリに取り込み clasp で同期する:

1. リポジトリ直下に `gas/` ディレクトリを新設: `gas/Code.gs`, `gas/appsscript.json`, `gas/.clasp.json` (scriptId はユーザーが記入)。
2. 初回はユーザーが現行 GAS エディタの内容を `gas/Code.gs` に手動コピー (Claude Code はコピー後の内容を正とする)。
3. デプロイ手順を `gas/README.md` に記載: `npm i -g @google/clasp` → `clasp login` → `clasp push` → GAS エディタで「デプロイを管理」から既存デプロイを更新 (URL を変えないため新規デプロイにしない)。
...
- optimize は §2.3.3 のグリッド全探索。統計画面にも「レシピ探索」セクション (`gp_explorer_section.dart`) を置き、産地×焙煎度を選ぶと湯温×比率の予測スコアヒートマップ (時間はグリッド上の μ 最大値で固定) を表示する。ヒートマップは fl_chart に無いため、`Table`+色付き `Container` (5℃×1.0比率の粗グリッド 4×5 に間引き) で実装する。
...
3. tQuantile(0.975, 10)=2.228139 (1e-4), tQuantile(0.975, 138)=1.977431 (1e-4)。
...
Phase 1 完了時点でユーザーの移行作業 (実データの名寄せ確定、GAS の scriptId 記入と clasp push) が必要。Claude Code は Phase 1 完了時に作業依頼をユーザーに明示すること。
