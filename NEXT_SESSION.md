...
> **【2026-08-10ユーザー指示、恒久】Antigravity CLI(agy)の正式運用移行が不具合対応を除く最重要タスク**。依存充足済みのT5-A41(パイロット導入・実機3回試用)を他の何より優先して選ぶ(`full_loop`スキルのタスク選定規則0に明記済み)。T5-A41完了後は`docs/antigravity_delegation_design.md` §9.5の状態遷移(パイロット→条件付き常時→常時委譲)を順に進める。
>
> **T5-A17の(b)再検証も優先**: `night_report.md`をリポジトリルートへ移動する修正(commit a634202)を行ったが未検証。次回の`night_loop.ps1`実行(有人試走 or 無人)で`night_report.md`が正しく生成・更新されるか確認する。通れば(a)(b)(c)すべて満たすためT5-A17を完了済みへ移せる。**T5-A12本体(タスクスケジューラへの23:00枠登録+3晩観察)は無人自動push運用の開始を意味する判断のため、着手前に必ずユーザーへ確認すること**(このスキル固有のタスク選定の自動承認の対象外として扱う)。
>
