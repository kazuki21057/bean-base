...
最終更新: 2026-08-05(`/full_loop`(**Opus 5**)、**T3-80a完了**=注湯ステップ・ハイライトずれ再発の検証要領書を作成。コード変更なし)
...
**2026-08-05: ユーザー報告「pouring stepのハイライトがまたずれている」(T3-79の再発/未解消)。4フェーズ運用(①上位で検証設計→②下位で検証→③上位で修正案→④下位で実装)の指示により、①=T3-80a(検証要領書`docs/pouring_highlight_verification_plan.md`)を作成・完了。**

**推奨着手順**:

1. **T3-80b(注湯ハイライトずれの検証実施、Sonnet 5向け)** — `docs/pouring_highlight_verification_plan.md`にそのまま従う。コード修正・デプロイ・push・本番書き込みは禁止
2. T3-75g(残豆量の分母不整合の補正方針、要ユーザー確認)
...
### -5.29 当日やったこと(2026-08-05、`/full_loop`(**Opus 5=上位モデル**)、**T3-80a完了**=注湯ハイライトずれ再発の検証要領書を作成)

- **経緯**: ユーザー報告「pouring stepのハイライトがまたずれている」(T3-79で2026-08-04に修正済みだったが再発/未解消)。あわせてユーザーから**4フェーズ運用**の指示: ①上位モデルで検証設計→②下位モデルで検証実施→③上位モデルで修正案検討→④下位モデルで実装。
- **成果物(コード変更なし)**: **`docs/pouring_highlight_verification_plan.md`**(検証要領書)。仮説H1〜H7、机上検証(本番`pouring_steps`実データ×`activeStepIndex`の手計算)+実機ブラウザ検証(スクショ12枚以内)の手順、報告テンプレ、禁止事項(コード修正・修正案の提案・デプロイ・本番書き込みの禁止)を規定。
- **調査で確定した前提**(要領書§2に記載済み、再調査不要): `lib/widgets/method_steps_editor.dart:67-101`の「経過時間」列は**そのステップの終了時刻(累計)**を表示している一方、`lib/utils/pouring_step_highlight.dart:17`の点灯区間は**[開始, 終了)** = 表示時刻の手前。この意味の不一致が最有力仮説(H1)。
- **タスク登録**: マスタープラン§3にT3-80a〜dを新規登録(a=本要領書=完了、b=検証、c=修正案検討(⚠️上位モデル)、d=実装)。
- **モデル使い分けについて**: `Agent`ツールの`model`パラメータ(`sonnet`/`opus`/`haiku`)で**サブエージェント単位のモデル指定が可能**とユーザーに回答済み。`.claude/agents/*.md`のfrontmatterで常設エージェント化もできる(現状このリポジトリに定義は無い)。
- **次にやること**: **T3-80b(検証実施)をSonnet 5で**。要領書のパスだけ渡せば単独で実行できる粒度になっている(サブエージェントに渡す場合は「日本語で報告して」を明示)。

> これ以前(-5.28節以前)の作業ログは **`docs/archive/NEXT_SESSION_log.md`** を参照。
