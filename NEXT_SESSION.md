...
最終更新: 2026-07-11(T2-2c・T2-1a・T2-1b・T2-3a〜c・T2-4a・T2-4b・T2-5a 完了、5時間セッション上限まで継続)

## -4.11 当日やったこと(2026-07-11、T2-5a・最新)
...
| ID | タスク | 依存 | サイズ |
|---|---|---|---|
| T2-5b | 評価時の豆残量自動計算と登録後の 031 復帰フロー | T2-5a ✅, T2-2b ✅ | M |
| T2-6 | スタッツ040の刷新 | T2-1a ✅ | L |
| T2-7 | 設定090(メインカラー・APIキー・データ保存先情報) | T2-1a ✅ | M |

推奨: T2-5b(評価時の豆残量自動計算と登録後の031復帰フロー)。マスタープラン§3のタスク表順で「依存充足済みの最上位タスク」。現状のT2-5aは`CoffeeRecord`を保存するだけで、登録後は`popUntil((route) => route.isFirst)`でダッシュボードへ単純に戻る実装になっている。T2-5bでは登録した豆量が残量計算(`calculateBeanRemainingPercent`は既にCoffeeRecordの`beanWeight`集計を見ているため、登録さえされれば自動的に反映されるはずだが実データで要確認)に反映されることと、「もう一杯記録する」的な031復帰導線(元の設計意図を`docs/Beanbase改修案.md`等で要確認、現状のマスタープランの一文だけでは詳細UX未確定)を実装する。

**着手前に推奨される確認(未実施が2件累積)**:
