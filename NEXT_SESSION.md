...
最終更新: 2026-08-05(`/full_loop`(Sonnet 5)、**T3-75c・T3-75d完了**=本番`coffee_data`ゴミ行3件・`filter_master`テストデータ削除)
...
1. T3-75g(残豆量の分母不整合の補正方針、要ユーザー確認。T3-72aの判断待ちでもある)
2. T3-75e(漢字グリフの初回描画時のみの豆腐化。優先度低に格下げ済み、対応するなら日本語フォントのプリロード/バンドル)
3. **T3-73e**(firebaseプラグイン・playwright MCPの無効化。グローバル設定に触れるため要ユーザー確認、設計書§5)。効果測定後のT3-73gはT3-73eの実測待ちで保留
4. T3-72aの扱いをユーザーに確認(クローズ→T3-72fへ統合 が妥当と思われる)
5. T3-72eの変更(コード整理のみ・表示挙動差分無し)をデプロイするかユーザーに確認(他タスクとまとめてでも可)

**T3-75c・T3-75dは2026-08-05に完了(下記「3. 直近の作業ログ」参照)。残るT3-75グループはT3-75e(優先度低)・T3-75g(要ユーザー確認)のみ。**
