...
最終更新: 2026-07-11(T2-2c・T2-1a・T2-1b・T2-3a・T2-3b・T2-3c・T2-4a 完了、5時間セッション上限まで継続)

## -4.9 当日やったこと(2026-07-11、T2-4a・最新)
...
推奨: T2-4b(「新規として保存」を021への継承遷移に変更)。マスタープラン§3のタスク表順で「依存充足済みの最上位タスク」。現状の030の`_saveAsNewSimulated`はSnackBar表示のみのシミュレーション実装なので、これを`MethodCreateScreen`(021)へ`editData`相当の値(新しい名前・基準豆量・ステップ一覧)を引き継いで遷移する形に置き換える。021側は既に`editData`引数を持つが、新規メソッド名以外の初期値(ステップ一覧等)を外部から渡す仕組みが無いため、コンストラクタ拡張が必要になる可能性がある。
