...
最終更新: 2026-07-25(`/loop 1h /full_loop`によるcron定期実行の4回目(loop_guardのバグ修正後、正しく$0からのコストで開始)として**T3-38(original-dataの本番Sheets移植)を完了**。着手前にPythonで既存本番データとの重複検証(ID一致・時刻の近接一致とも)を徹底し、新規`tools/migrate_original_data.dart`(冪等、既存ID確認のうえ未登録分のみ追加)で移植を実施。本番bean_master 23→27件、coffee_data 146→173件。詳細は直下の-4.57節。**残る新規タスクはT3-39(Geminiモデル選択設定, M, 依存なし・優先度低)のみ。既存のT3-1/T3-4/T3-20はユーザー作業主体。** 次回`/loop`起動時はこのファイルと`docs/改修マスタープラン.md` §3から着手タスクを選ぶこと。)
