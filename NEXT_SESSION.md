...
最終更新: 2026-07-11(T2-2c・T2-1a・T2-1b・T2-3a・T2-3b・T2-3c 完了、5時間セッション上限まで継続)

## -4.8 当日やったこと(2026-07-11、T2-3a・T2-3b・T2-3c・最新)
...
| ID | タスク | 依存 | サイズ |
|---|---|---|---|
| T2-4a | 030 からの Pouring Steps 編集(既存メソッド更新) | T2-3b ✅ | M |
...
推奨: T2-4a(030からのPouring Steps編集を実際にSheetsへ保存)。マスタープラン§3のタスク表順で「依存充足済みの最上位タスク」であり、今回030に実装した「メソッドを保存」ダイアログ(現状シミュレーションのみ)を実際の`DataService.updateMethod`/`updatePouringStep`/`addPouringStep`/`deletePouringStep`接続に置き換える(021の`MethodCreateScreen._submit`と同じ差分パターンが流用できる)。**ID重複追加バグに注意**: `PouringStep.id`が`'new_'`プレフィックスの間は「未保存」の目印として扱われているため、保存成功後にIDを`'new_'`以外の確定IDへ更新してローカル状態を更新し直さないと、同じ画面を開いたまま2回目の保存をした際に同じステップが`addPouringStep`で二重追加されてしまう(021は保存後に画面を閉じるためこの問題が顕在化しないが、030は保存後も画面が開いたままなので要注意)。T2-4b着手時は「新規として保存」を021への遷移(pre-filled)に変更する必要がある(現在の030内で完結する簡易実装ではなく)。
