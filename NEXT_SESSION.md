...
最終更新: 2026-07-26(**ユーザーから不具合・要望を受領し、T3-40〜T3-53(14件)としてマスタープラン§3に追加(実装は未着手、記録・整理のみ)**。詳細は直下の-4.60節。**着手前の確認事項3件は同日中にすべて解消済み**: 焙煎度は8段階採用(+説明ページT3-51を追加)・テスト削除は承認取得・「A/Bテスト」は「同じ豆で最適なメソッド/湯温/粒度を知る+検証状況の可視化」が狙いと判明しT3-52/T3-53として再設計。**すぐ着手できる(依存なし)のはT3-44(S)・T3-45(M)・T3-42(L)・T3-46(S)・T3-50(M)**。`/loop`のcronは停止中(再開はユーザー指示待ち)。)
