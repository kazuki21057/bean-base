...
最終更新: 2026-07-31(`/full_loop`(Sonnet 5)、**T3-43完了・本番デプロイ・push済み(ただし実ブラウザでのGemini呼び出しは未検証)**=豆情報AI自動入力の焙煎度enumValues/プロンプトを8段階へ更新。次回は T3-49 / T3-69 のいずれかから)
...
- 本番: https://beanbase-app-2016.web.app (Firebase Hosting)。**T3-43のビルドはデプロイ済み・ハッシュ一致確認済み。working tree はクリーン、git push も完了済み**(2026-07-31時点)。
- ストレージはGoogle Sheets+Drive(GAS Web App経由)。GAS は `gas/Code.gs` を clasp で管理。
- **T3-43(豆情報AI自動入力の焙煎度enumValues/プロンプトを8段階へ更新)は2026-07-31に完了・検証・本番デプロイ済み**。詳細は下記「3. 直近の作業ログ」を参照。**実ブラウザでのGemini呼び出し自体は未検証**(ローカル環境にAPIキー未設定+Flutter WebのファイルピッカーがOS標準ダイアログ経由でブラウザ拡張から操作できず、画像アップロード→AI抽出の実行まで到達できなかった)。**ユーザーが実機/ローカルで、英語表記または旧5段階の日本語表記のパッケージ画像を使い、012のAI自動入力で焙煎度が正しく反映されることを確認してほしい。**
