...
最終更新: 2026-07-09(T1-5a・T1-5b 完了)
...
推奨: T1-5c(グラインダー、Sサイズ)が最も軽い。ドリッパー(`lib/screens/dripper_*.dart`)・フィルター(`lib/screens/filter_*.dart`)と全く同じパターンで実装できる(GrinderMasterのフィールドは name/grindRange/description/imageUrl)。T1-5dはメソッドの抽出回数集計・発案者表示など差分吸収が必要なのでMサイズ。
