...
最終更新: 2026-07-10(T1-6a 完了)

## 0. 当日やったこと(2026-07-10、続き)
...
- ブラウザでの目視確認はサンドボックス制約のため未実施(前回までと同じ既知の制約)。**次回ユーザーがローカルで `flutter run -d chrome` を実行し、010(豆一覧のカード表示・0%表示切替・詳細へのタップ遷移)を目視確認することを推奨。**
- マスタープラン進捗表を更新: §3 の T1-6a を ✅、§4 の 010 を ✅ に変更。あわせて前回(T1-5d)完了時に更新漏れだった §4 の 019/020/021 も ✅ に修正。
- commit/push 予定(このセッション内、T1-6a 単独コミット)。
...
推奨: T1-6a完了により`lib/screens/bean_list_screen.dart`が実データで動作。次はT1-6b(豆詳細011・新規豆012)。011は`MasterDetailTemplate`の`extraSections`(fields と関連履歴の間に任意ウィジェットを挿入)が使える可能性が高いが、豆固有の「残量%表示」は実際の計算ロジック(T2-2b)が未実装なので、T1-6aと同様に`isInStock`ベースの暫定表示にとどめること。012は`BeanCreateScreen`(現状UIモック、保存未接続)を`DripperCreateScreen`相当のDataService接続版に書き換える形になる見込み。
