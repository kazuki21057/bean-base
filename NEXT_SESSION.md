...
| ID | タスク | 依存 | サイズ |
|---|---|---|---|
**Phase 2は2026-07-11に全タスク完了。次はPhase 3(Cycle 27〜、軽微な修正・仕上げ)。T3-10は2026-07-18に完了。**
...
| T3-11 | Firebase Hosting 環境構築(**初回ログインはユーザー操作**) | T3-10 ✅ | M |
...
推奨: T3-9(メインカラーの反映範囲拡大)。マスタープラン§3のタスク表順で依存充足済みの最上位タスク(T2-7・T2-1aとも✅)。ただしサイズLで技術的制約に当たる可能性がある点に注意(終了条件に「対応不可な範囲を明記しユーザーの判断を仰ぐ」の逃げ道あり)。T3-11(Hosting構築)も依存(T3-10)が satisfiedになったため並行候補だが、初回`firebase login`にユーザー操作が必要な点に注意。
