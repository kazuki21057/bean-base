...
最終更新: 2026-07-26(**ユーザーから不具合・要望11件を受領し、T3-40〜T3-50としてマスタープラン§3に追加(実装は未着手、記録・整理のみ)**。詳細は直下の-4.60節。あわせて、新規タスクが枯渇したため前日に停止していた`/loop`のcronは停止したまま(再開する場合はユーザーの指示待ち)。**次に着手するなら不具合系のT3-44(登録ボタンが押せない、S)またはT3-45(一覧反映が遅い、M)が依存なしで着手しやすい。T3-40(カメラ起動しない)は症状のヒアリングが先。** **着手前にユーザー確認が必須な3件に注意: T3-42(焙煎度9段階の内訳 — 業界標準は8段階)・T3-46(本番データ削除の承認)・T3-50(A/Bテストの仕様が未定義)。**)
...
- **着手前にユーザー確認が必要な3件(タスク文にも明記)**: ①**T3-42** 焙煎度9段階の内訳(8段階に何を足すか) ②**T3-46** 本番データ削除の対象一覧承認(このプロジェクトでは削除のみが例外的に要確認) ③**T3-50** 「A/Bテスト」で何を比較しどう動くのかの仕様。
- **変更ファイル**: `docs/改修マスタープラン.md`/`NEXT_SESSION.md`。コード変更なし。
- **次回の着手点**: 依存なしですぐ着手できるのは**T3-44(S、SnackBarが保存ボタンに重なっている疑い)**と**T3-45(M、まず実測から)**。T3-40はユーザーへの症状ヒアリングが先。
