...
最終更新: 2026-07-28(ユーザー要望6件をT3-58〜T3-69としてタスク登録のみ実施、コード変更なし)。**詳細は-4.70節。要点**: 豆の追加購入・購入履歴・購入店マスタ・保存場所・残量手動調整・030の湯量スケーリング不具合を12タスクに細分化して`docs/改修マスタープラン.md`に登録した。**T3-61(追加購入+購入履歴の統合設計)とT3-66(購入店マスタ設計+Web情報収集)のみ上位モデル指定、それ以外10件はSonnet 5で`/full_loop`実行可能な粒度まで詳細化済み。** 次に着手できる(依存なし)のは **T3-58(S、原因調査済み)・T3-59(M)・T3-60(M)** および従来からの T3-46(S、残4件)・T3-50(M)・T3-43(L)・T3-51(M)・T3-47(M)。**T3-45の本番デプロイが依然未実施**(前回`firebase deploy`がハーネスの安全分類器にブロックされたため、-4.69節参照)。
