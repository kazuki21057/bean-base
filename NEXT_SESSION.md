...
最終更新: 2026-08-04(`/full_loop`(Sonnet 5)、**T3-75i完了・本番デプロイ・push済み**=抽出履歴詳細でマスタ取得失敗時に生IDでなく「不明」表示)
...
**2026-08-04: T3-75i完了・本番デプロイ・push済み**(抽出履歴詳細(003)でマスタ取得失敗時にドリッパー等の生IDがそのまま表示される問題を修正。詳細は下記「3. 直近の作業ログ」)。

**推奨着手順**:

1. T3-75c / T3-75d / T3-75g は本番データの削除・補正を伴うためユーザー確認から
2. T3-75e(漢字グリフの初回描画時のみの豆腐化。優先度低に格下げ済み、対応するなら日本語フォントのプリロード/バンドル)
3. **T3-73e**(firebaseプラグイン・playwright MCPの無効化。グローバル設定に触れるため要ユーザー確認、設計書§5)。または依存が満たされ確認不要な**T3-73f**(CLAUDE.md圧縮)に着手可
