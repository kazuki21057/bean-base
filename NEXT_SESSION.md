...
**サブエージェント(2026-08-05新設)**: `.claude/agents/verifier.md`(検証専任・Sonnet固定)/`.claude/agents/implementer.md`(実装専任・Sonnet固定)。上位モデルで起動したセッションでは、検証・実装をこれらに委譲すればモデル分担ルールを自動的に満たせる(`subagent_type: verifier` / `implementer`)。
